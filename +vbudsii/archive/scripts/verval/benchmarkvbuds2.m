...
VBUDS_flux3 = VBUDS_flux3*max(max([r.Cell(1).spectralFlux r.Cell(2).spectralFlux]))/max(max(VBUDS_flux3));
%{
...
if sum(r.Cell(1).spectralFlux ~= vbudsflux(:,2)) || ...
    ...
    disp('benchmarkvbuds2 flux test FAILED');
    figure;
    subplot(1,2,1);
    ...
    %print('benchmarkvbuds2_111026.eps','-depsc','-r300');
else
    disp('benchmarkvbuds2 flux test passed');
%    [Region(1).Cell(1).spectralFlux == phi(:,1), ...
%    Region(1).Cell(2).spectralFlux == phi(:,2)]
end

if sum(r.Cell(1).spectralFlux ~= VBUDS_flux3(:,2)) || ...
    sum(r.Cell(2).spectralFlux ~= VBUDS_flux3(:,1))
    disp('benchmarkvbuds3 flux test FAILED');
    figure;
    subplot(1,2,1);
    loglog(p.fineGroupDef(1:end-1)'*[1 1],[r.Cell(1).spectralFlux ...
        VBUDS_flux3(:,2)])
    ...
    xlabel('E (eV)')
    legend('vbudsii','vbudsi','Location','Best')
    [r.Cell(2).spectralFlux./VBUDS_flux3(:,2) r.Cell(2).spectralFlux-vbudsflux(:,2)]
    %print('benchmarkvbuds2_111026.eps','-depsc','-r300');
else
    disp('benchmarkvbuds3 flux test passed');
%    [Region(1).Cell(1).spectralFlux == phi(:,1), ...
%    Region(1).Cell(2).spectralFlux == phi(:,2)]
end
