function [Ri] = CreatePi4(L, p, r, Ri)
...
% Initialize.
PIhomo=ones(r.nCells, r.nCells, p.nFineGroups);
...
EscapeAAnti = EscapeAntipin(iA, :);
EscapeBAnti = EscapeAntipin(iB, :);
EscapeAPin = EscapePin(iA, :);
EscapeBPin = EscapePin(iB, :);

TransmitAAnti = TransmitAntipin(iA, :);
TransmitBAnti = TransmitAntipin(iB, :);
TransmitAPin = TransmitPin(iA, :);
TransmitBPin = TransmitPin(iB, :);

% this is new, having four tau's.
denomA = 1 - TransmitAAnti .* TransmitAPin;
denomB = 1 - TransmitBAnti .* TransmitBPin;
PathAAnti = (1 - TransmitAAnti) ./ denomA;
PathBAnti = (1 - TransmitBAnti) ./ denomB;
PathAPin = (1 - TransmitAPin) ./ denomA;
PathBPin = (1 - TransmitBPin) ./ denomB;

PIhetero = zeros(r.nCells, r.nCells, p.nFineGroups);

includeInfinite = 1;
if includeInfinite
    % cell-pair A
    % diagonals (self-collision)
    PIhetero(iAAnti,iAAnti,:) = squeeze(...
        1 - EscapeAAnti + EscapeAAnti .* TransmitAPin .* PathAAnti);

    PIhetero(iAPin,iAPin,:) = squeeze(...
        1 - EscapeAPin + EscapeAPin .* TransmitAAnti .* PathAPin);
    
    % offdiag
    PIhetero(iAAnti,iAPin,:) = squeeze(...
        EscapeAPin .* PathAAnti);

    PIhetero(iAPin,iAAnti,:) = squeeze(...
        EscapeAAnti .* PathAPin);

    plotstuff = 0;
    if plotstuff
        disp('hey check it out');
        figure; subplot(2,2,1); hold on;
        plot([(1 - EscapeAAnti + EscapeAAnti .* TransmitAPin .* PathAAnti)', ...
            (1 - EscapeAAnti)']);
        ylabel('AAnti to AAnti');
        legend('stay plus transmit', 'stay');
        title(['ThreshFac: ' num2str(p.homogenizeThreshFac)]);
        disp('hey check it out');
        subplot(2,2,4); hold on;
        plot([(1 - EscapeAPin + EscapeAPin .* TransmitAAnti .* PathAPin)', ...
            (1 - EscapeAPin)']);
        ylabel('APin to APin');
        legend('stay plus transmit', 'stay');
        disp('hey check it out');
        subplot(2,2,2); hold on;
        plot([(EscapeAPin .* PathAAnti)', EscapeAPin']);
        ylabel('APin to AAnti');
        legend('P tau', 'P');
        title([' slopeFac: ' num2str(p.homogenizeSlopeFac)]);
        disp('hey check it out');
        subplot(2,2,3); hold on;
        plot([(EscapeAAnti .* PathAPin)', EscapeAAnti']);
        ylabel('AAnti to APin');
        legend('P tau', 'P');
    end

    % cell-pair B
    % diagonals
    PIhetero(iBAnti,iBAnti,:) = squeeze(...
        1 - EscapeBAnti + EscapeBAnti .* TransmitBPin .* PathBAnti);

    PIhetero(iBPin,iBPin,:) = squeeze(...
        1 - EscapeBPin + EscapeBPin .* TransmitBAnti .* PathBPin);
    % offdiag
    PIhetero(iBAnti,iBPin,:) = squeeze(...
        EscapeBPin .* PathBAnti);

    PIhetero(iBPin,iBAnti,:) = squeeze(...
        EscapeBAnti .* PathBPin);
else
    % cell-pair A
    % diagonals (self-collision)
    PIhetero(iAAnti,iAAnti,:) = squeeze(...
...
% Wedding!
for iBin = 1:p.nFineGroups
    PI(:,:,iBin) = Homo(iBin) * PIhomo(:,:,iBin) + ...
        (1 - Homo(iBin))* PIhetero(:,:,iBin);
end
