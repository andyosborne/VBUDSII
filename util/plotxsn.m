function g = plotxsn(binit,macro,mytitle,legends,xbelow,groupdef,varargin)

if binit == 1
    lengthX = length(groupdef) - 1;
    XaxisXS = zeros(2*lengthX,1);
    XaxisXS(1) = groupdef(1);
    for i = 2:lengthX
       XaxisXS(2*i-2:2*i-1) =  groupdef(i)*[1; 1];
    end
    XaxisXS(end) = groupdef(end);
else
    XaxisXS = groupdef(1:end-1)';
end
% prepare for bin-wise plotting.
plotn = zeros( length(XaxisXS),nargin-6);
minval = inf;
maxval = -inf;
for i = 1:(nargin-6)
    if binit == 1
        plotn(:,i) = binwise(varargin{i});
    else
        plotn(:,i) = varargin{i};
    end
