...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Note to self.  Fill matricies with NaN rather than zero if they don't have
%corresponding data.
        ...
        if Leg
           for j=1:1:length(M) 
             if ~sparsed 
                ...
                for s1=1:length(B.s0s)
                   B.m(index.mt).p(index.tmp,s1,M(j,3)+1).kern(1:B.nGroups,1:B.nGroups) = sparse(0);
    ...
    case {'matx1','6_51'} %Find the matrix infinite dilution only     
        ...
        [Dy,Dx]=size(M);
         ...
         if ~sparsed 
             ...
             B.m(index.mt).p(index.tmp).kern=sparse(Z); 
         end   
