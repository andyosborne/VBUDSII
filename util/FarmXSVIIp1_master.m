function [Meta, L, p] = FarmXSVIIp1_master(p)
  ...
  %  disp(fline)
    if length(fline) > 12 && ~isempty(strfind(fline, '<TD><I><B>'))
        ...
        if ~isempty(strfind(fline, '<B>-'))
            ...
            if Meta(iM).element=="TC"
                Meta(iM).numProtons =43;
            end
        else
   ...
   % Ex: <TD><A HREF="/data/data/ENDFB-VII.1-neutron/Ho/165">raw eval</A></TD>
        ...
        if Meta(iM).element=="TC"
            appender='/data/data/ENDFB-VII.1-neutron/Tc/99';

        else
            appender=fline(iLeft(1):iRight(1));
        end
            
        Meta(iM).webPath = [t2domain '/nis' appender];
...
while (~contains(fline, 'group'))  && ~contains(fline, '*****************************************************************************') % Andy edit
