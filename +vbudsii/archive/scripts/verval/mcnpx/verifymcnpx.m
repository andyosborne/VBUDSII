...
g.regionDef(1).cellDef(1).initTemp = 600;
% UO2 cell
...
g.regionDef(1).cellDef(2).initTemp = 600;
...
% MCNPX CROSS SECTIONS
tallyxs = TallyPull2('xs1/outp');
...
if sum(r.Cell(1).spectralFlux ~= mcnpflux(:,1)) || ...
    ...
    legend('vbudsii','mcnpx','Location','Best')
    axis([0 inf 0 1e14])
    [r.Cell(2).spectralFlux./mcnpflux(:,2) ...
    r.Cell(2).spectralFlux-mcnpflux(:,2)];
...
%    fprintf(fid,'FM34 -1 100 (1) (2) (18) (102) (-6 -7)\n');
    for cellidx = 1:g.regionDef(1).nCells
        ...
        for j = 1:4
            ha = subplot(2,2,j);
            if (j == 4) && ~g.regionDef(1).cellDef(cellidx).isFissile
            else
